...
export async function PATCH(request: Request) {
  try {
    ...
    const embeddingDimension =
      typeof body.embedding_dimension === "string" && body.embedding_dimension.trim() === ""
        ? undefined
        : typeof body.embedding_dimension !== "undefined"
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Unable to save AI configuration." },
      { status: 500 }
    );
  }
}
