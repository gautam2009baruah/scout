...
export async function GET(request: Request) {
  ...
  if (!hasModuleAccess(session, MODULE_KEYS.triggersMonitoring)) {
    return NextResponse.json({ message: "You do not have permission to view triggers monitoring." }, { status: 403 });
  }
