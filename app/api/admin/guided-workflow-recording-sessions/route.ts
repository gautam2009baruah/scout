...
import { createGuidedWorkflowRecordingSession, GuidedWorkflowError, listGuidedWorkflowRecordingSessions } from "@/lib/admin/guided-workflows";
...
export async function GET() {
  ...
  return NextResponse.json({ sessions: await listGuidedWorkflowRecordingSessions(auth.session) });
}
...
export async function POST(request: Request) {
  ...
  if (!body || typeof body !== "object") {
    return NextResponse.json({ message: "Recording session payload is required." }, { status: 400 });
  }
